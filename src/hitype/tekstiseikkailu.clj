...
(defn mahdolliset-komennot [maailma]
  (:komennot (pelaajan-paikka maailma)))
...
(defn aloita []
  (loop [maailma (alusta-maailma)]
    (let [paikka (pelaajan-paikka maailma)
          komentokartta (komentokartta (mahdolliset-komennot maailma))]
      (println "-------------------------")
      (when-let [tapahtuman-kuvaus (:tapahtuman-kuvaus maailma)]
        (println tapahtuman-kuvaus)
        (println))

      (println (:nimi paikka))
      (println (:kuvaus paikka))

      (when (not (empty? (:tavarat maailma)))
        (println "Sinulla on:")
        (doseq [tavara (:tavarat maailma)]
          (println (str "  " (:nimi tavara)))))

      (println)
      (println "Komennot:")
      (doseq [[näppäin komento] komentokartta]
        (println (str (.toUpperCase näppäin) " : " (:kuvaus komento))))
      (let [komento (valitse-komento komentokartta)]
        (recur ((:toteutus komento) (dissoc maailma :tapahtuman-kuvaus)))))))
